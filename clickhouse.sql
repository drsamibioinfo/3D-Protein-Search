-- Proteins Table
create table proteins (`protein_id` String , `size` UInt64, `helices` UInt64, `sheets` UInt64,`loops` UInt64,`bends` UInt64,`sequence` String,`ss` String, `pattern` String,`processdate` Date DEFAULT today()) Engine=MergeTree(processdate,(protein_id,helices,sheets,loops,bends,sequence,ss,pattern),8192);
-- Helix
